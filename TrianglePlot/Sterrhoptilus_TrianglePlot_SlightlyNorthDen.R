#Triangle Plot with two slightly north dennistouni
...
locs<-read.csv("./Data/Sterrhoptilus_SamplingDataSubsetted_NoNorthernDen.csv")
...
vcf@gt<-vcf@gt[,c(1,11:24,38,39,43:46)]
...
colnames(conv.mat)
#two S. dennistouni from slightly outside hybrid area are 21084 and 21112, or columns 13 and 14.
#calc AF for the samples that will be used to call fixed differences
denn.af<-(rowSums(conv.mat[,c(13,14)], na.rm=T)/(rowSums(is.na(conv.mat[,c(13,14)]) == FALSE)))/2
nigr.af<-(rowSums(conv.mat[,c(17,18,19)], na.rm=T)/(rowSums(is.na(conv.mat[,c(17,18,19)]) == FALSE)))/2
...
for (i in 1:nrow(gen.mat)){
  ...
  if((sum(conv.mat[i,c(13,14)], na.rm=T)/(sum(is.na(conv.mat[i,c(13,14)]) == FALSE)))/2 < .2){
...
#plot triangle
plot(x=hi.index.sim$h, y=het, bg=c(rep(rgb(0.9568627450980393, 0.7176470588235294, 0.10980392156862745,alpha=.5), times=16), rep(rgb(0.43529411764705883,0.43529411764705883,0.43529411764705883,alpha=.5), times=4)),
     pch=21, cex=1.5,
